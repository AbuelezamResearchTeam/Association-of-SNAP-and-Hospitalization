...
for (i in outcomes) {
  ...
  synth<-df%>%
    synthetic_control(outcome = !!sym(i),
                      ...
                      i_time = 0,
                      generate_placebos=T)%>%
    generate_predictor(time_window = -3:3,
                       white_m=mean(white),
                       asian_m=mean(asian),
                       black_m=mean(black),
                       other_m=mean(other),
    ...
    generate_predictor(time_window = -3:0,
                       gen_health_good=mean(gen_health_good),
                       outcome_m=mean(!!sym(i)))%>%
    generate_weights(optimization_window = -3:0,
...
#save----
rm(df,outcomes,i)
save(list=ls(),file=here("trunk","analysis","Synthetic Control","tidysynths.RData"))
rm(list=ls())
